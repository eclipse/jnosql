...
class CassandraReturnQueryAsync implements Runnable {
    ...
    public CassandraReturnQueryAsync(ResultSetFuture resultSet, Consumer<List<ColumnFamilyEntity>> consumer) {
    ...
    public void run() {
        try {
            ...
            List<ColumnFamilyEntity> entities = resultSet.all().stream().map(row -> CassandraConverter.toDocumentEntity(row)).collect(Collectors.toList());
