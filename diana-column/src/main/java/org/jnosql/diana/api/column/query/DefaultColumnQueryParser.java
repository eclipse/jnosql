...
public class DefaultColumnQueryParser implements ColumnQueryParser {
    ...
    public List<ColumnEntity> query(String query, ColumnFamilyManager manager) {
        ...
        switch (command) {
            case "select":
                return select.query(query, manager, this);
            case "delete":
                return delete.query(query, manager, this);
            case "insert":
                return insert.query(query, manager, this);
            case "update":
                return update.query(query, manager, this);
            default:
    ...
    public ColumnPreparedStatement prepare(String query, ColumnFamilyManager manager) {
        ...
        switch (command) {
            case "select":
                return select.prepare(query, manager, this);
            case "delete":
                return delete.prepare(query, manager, this);
            case "insert":
                return insert.prepare(query, manager, this);
            case "update":
                return update.prepare(query, manager, this);
            default:
                throw new QueryException(String.format("The command was not recognized at the query %s ", query));
        }
    }
    ...
    private void validation(String query, ColumnFamilyManager manager) {
